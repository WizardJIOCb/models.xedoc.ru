...
function modelCard(model) {
  ...
  return `<article class="community-model-card"><a class="community-model-cover" href="/model/${encodeURIComponent(model.id)}" aria-label="Открыть модель: ${escape(modelTitle(model))}"><span class="community-model-fallback" aria-hidden="true">${icon('cube')}<span>3D / MODEL</span></span>${model.previewUrl ? `<img src="${escape(model.previewUrl)}" alt="${escape(modelTitle(model))}" loading="lazy" decoding="async" />` : ''}<span class="community-cover-label">${model.hasRig ? 'Скелет · GLB' : 'GLB · 3D'}</span><span class="community-cover-open">Смотреть ${icon('arrow')}</span></a><div class="community-model-info"><a class="community-model-title" href="/model/${encodeURIComponent(model.id)}">${escape(modelTitle(model))}</a><div class="community-model-meta">${author ? `<a class="community-author" href="${profileUrl(author)}">${avatar(author)}<span>${escape(displayName(author))}</span></a>` : '<span class="community-author community-author-anonymous">Автор без профиля</span>'}<a class="community-comment-count" href="/model/${encodeURIComponent(model.id)}#comments" aria-label="${escape(plural(model.commentsCount, 'комментарий', 'комментария', 'комментариев'))}">${icon('comment')}<span>${Number(model.commentsCount) || 0}</span></a></div></div></article>`;
}
